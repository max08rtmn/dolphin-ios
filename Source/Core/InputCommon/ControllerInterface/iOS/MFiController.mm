...
{
static void LogJoyConFixControllerDetails(GCController* controller)
{
  NSLog(@"[JoyConFix] controller='%@' extended=%d micro=%d gamepad=%d motion=%d",
        controller.vendorName, controller.extendedGamepad != nil, controller.microGamepad != nil,
        controller.gamepad != nil, controller.motion != nil);

  if (controller.extendedGamepad != nil)
  {
    GCExtendedGamepad* gamepad = controller.extendedGamepad;
    NSLog(@"[JoyConFix] extended fields: LShoulder=%d RShoulder=%d LTrigger=%d RTrigger=%d "
          "Menu=%d Options=%d Home=%d",
          gamepad.leftShoulder != nil, gamepad.rightShoulder != nil, gamepad.leftTrigger != nil,
          gamepad.rightTrigger != nil, gamepad.buttonMenu != nil, gamepad.buttonOptions != nil,
          gamepad.buttonHome != nil);
  }
  ...
  if (@available(iOS 14.0, *))
  {
    ...
    NSLog(@"[JoyConFix] physical buttons count=%lu", (unsigned long)buttons.count);
    for (NSString* name in buttons)
      NSLog(@"[JoyConFix] physical button='%@'", name);
  }
}
...
{
  LogJoyConFixControllerDetails(controller);
