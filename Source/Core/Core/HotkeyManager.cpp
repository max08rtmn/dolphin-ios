...
void LoadConfig()
{
  ...
  auto* hotkey_manager = static_cast<HotkeyManager*>(s_config.GetController(0));
  const int screenshot_group = hotkey_manager->FindGroupByID(HK_SCREENSHOT);
  hotkey_manager->GetHotkeyGroup(static_cast<HotkeyGroup>(screenshot_group))
      ->controls[hotkey_manager->GetIndexForGroup(screenshot_group, HK_SCREENSHOT)]
      ->control_ref->SetExpression("F9 | `Capture` | `Screenshot` | `Raw Button Capture` | "
                                   "`Raw Capture` | `Raw Button Screenshot` | `Raw Screenshot` | "
                                   "`Raw Button Share` | `Raw Share` | `Raw Button Create` | "
                                   "`Raw Create`");
  hotkey_manager->UpdateReferences(g_controller_interface);
}
...
void HotkeyManager::LoadDefaults(const ControllerInterface& ciface)
{
  ...
  set_key_expression(HK_SCREENSHOT, "F9 | `Capture` | `Screenshot` | `Raw Button Create` | `Raw Create`");
